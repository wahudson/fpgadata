...
=item B<--flag>             - Binary flags with 8-bit data in hex.
...
Flags are 4-bits in binary.  Bit positions left to right are:

    #1000 NoData (fifo empty)
    #0100 OverFlow (fifo write_full)
    #0010 MarkY scan mark
    #0001 MarkX scan mark
...
=item B<--freerun>          - Keep all samples, ignoring NoData.

Ignore the NoData signal and continue thru any operation steps that might
hang.  Use as a debug mode to capture a free running stream of input samples.
Typically display result with --flag and look for stuck signal lines (bits).
...
Trace key operation steps with text preceeded by a '+' character on stderr.
...
=head1		EXAMPLES
 ...
 Free run data capture, to debug stuck signal lines.
 % rpixel --freerun  --flag

 Output raw data with flag field in binary.  Useful for debug.
 % rpixel --flag
...
=head2			Hang in execution

Can be caused by stuck handshake (control) signals.  Commonly occurs in
step (1c) "Find first coefficient".
Use --debug to see operation trace comments (preceeded by a '+' character).
Use --freerun to continue thru potential hangs, and --flag to show control
signals and data.  Look for stuck signal lines (bits) in the flags, and in
the coefficient number field (upper 4-bits of the 8-bit data).
